...
function IconBookmark({data, onBookmark, setBookmark}) {
    return (
        <div className="bg-white absolute bottom-[-15px] left-4" 
            onClick={() => setBookmark(data)}>
                {/* // console.log('bookmark on', data) */}
                <img 
                src={onBookmark.id === data.id ? "bookmark_selected.png" : "bookmark_unselected.png"} 
                alt={onBookmark.id === data.id ? "bookmark_selected" : "bookmark_unselected"} 
                height={30} 
                width={30} />
            </div>  
    )
